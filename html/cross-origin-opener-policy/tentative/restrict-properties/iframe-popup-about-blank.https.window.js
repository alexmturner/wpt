...
function iframePopupAboutBlankTest(
  ...
  promise_test(async t => {
    ...
    // Navigate to the same origin as the initial window, with COOP:RP and
    // COEP: require-corp. Expect to be crossOriginIsolated.
    const popup_token = token();
    const popup_src = same_origin + executor_path +
        coop_restrict_properties_header + coep_require_corp_header +
        `&uuid=${popup_token}`;
